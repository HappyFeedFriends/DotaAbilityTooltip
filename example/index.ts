...
registerAbilitiesTooltip({
    ...
    onShown: (panel, abilityIndex) => {
        ...
        panel.SetDialogVariable(
            "abilityDescription",
            $.Localize(`#Dota_tooltip_ability_${Abilities.GetAbilityName(abilityIndex)}_description`)
        );
        panel.SetDialogVariable(
            "lore",
            $.Localize(`#Dota_tooltip_ability_${Abilities.GetAbilityName(abilityIndex)}_lore`)
        );
